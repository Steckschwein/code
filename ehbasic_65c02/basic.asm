...
; Original version/source by Lee Davison w/updates to Version 2.22
; Patches and updates by Klaus Dorman Version 2.22p4 - see below
; "C" is appended to define CMOS instructions/addressing modes used
; Modified source code to assemble with WDC Tools package
; Changes by K. E. Maier - July-November 2018
;   minor changes and cleanup as of 13th November 2018
;
; Additional functions added 25th September 2019
;   LOAD/SAVE functions via Xmodem-CRC routines in C02Monitor Version 2.03
;
; Additional patches added. Klaus provided a 2.22p5 patched version 13th January 2020
; those patches are included here, albeit not in identical code for all patches
;
; Several of the Basic messages have been abbreviated to keep ROM space below $2700 bytes
;
...
;  Use ROM based CHRGET/CHRGOT routine with Page Zero pointer
;  All Page zero usage is from the bottom ($00) up ($AF is max available)
;  CMOS opcodes/addressing modes are used! tested with WDC65C02 only!
;  Version renamed to Ver 2.22p5C (concurrency with Klaus' patched version)
; The following functions and all associated code (interrupt related)
...
; - RETNMI
; - OFF
; NOTE: BASIC Tokens are NOT the same due to removal of the above commands!!
;
; An EXIT primary command has been added
;  The EXIT command performs a JMP to the C02Monitor Warm start vector
;  EhBasic can be re-entered via Warm start unless pages zero/four are changed
;
...
; 2.22p4C   Port to C02 Pocket SBC - SCC2691 UART Console, 32KB RAM / 32KB ROM, 6MHz CPU
;
...
;STACK_RES         = #20       ; reserved stack space
STACK_RES         = 20       ; reserved stack space
...
; new page $04 initialisation, copy ROM data to RAM
; consists of the vectors Input/Output/Load/Save/Exit
; and Control-C flags, pointers

LAB_COLD
      LDX   #PG4_TABE-PG4_TABS-1 ; byte count-1
LAB_2D13
      LDA   PG4_TABS,X           ; get byte
      STA   ccflag,X             ; store in page 4
      DEX                        ; decrement count
      BPL   LAB_2D13             ; loop if not done

      TXS                        ; reset stack pointer (X Reg = $FF from above)
...
LAB_2D93
      ...
      STA   (Itempl)          ; save via temporary integer
      CMP   (Itempl)          ; compare via temporary integer
...
LAB_2DB6
      ...
      INY                     ; increment Ram_base low byte
...
LAB_1212                      ; patched for stack floor
      CLC                     ; prep ADC
      ADC   #STACK_RES        ; stack pointer lower limit before interrupts
...
LAB_INLN
      ...
      JSR   LAB_18E0          ; print " "
      BRA   LAB_1357          ; call for BASIC input and return
...
LAB_134B
      ...
      BRA   LAB_1359          ; branch over next instruction, save 1 clock cycle
...
LAB_137F
      JSR   LAB_PRNA          ; go print the character
      BRA   LAB_1359          ; always loop for next character
...
LAB_138E
      ...
      BRA   LAB_137F          ; go print the [BELL] but ignore input character
                              ; branch always
...
LAB_1456
      ...
      BRA   LAB_SHLN          ; go search for line # in temp (Itempl/Itemph) from AX
                              ; (carry always clear)
...
LAB_FOR
      ...
      JSR   LAB_27BA          ; round FAC1 First (patch 2.22p5)
...
LAB_RESTOREn
      ...
      BRA   LAB_go_search     ; branch always (can never be carry clear)
...
LAB_line_found
      ...
      BRA   LAB_uflow         ; else decrement high byte then save DATA pointer and
...
LAB_RUN
      ...
      JMP   LAB_1477          ; reset execution to start, clear variables, flush stack, return

; does RUN n

LAB_1696
      JSR   LAB_147A          ; go do "CLEAR"
      BRA   LAB_16B0          ; get n and do GOTO n (branch always as CLEAR sets Z=1)
...
LAB_16B0
      ...
      JMP   LAB_15C2          ; go do interpreter inner loop, return
...
LAB_GOTO
      ...
      INX                     ; increment high byte
      BRA   LAB_16D4          ; branch always (can never be carry)
...
LoopDone
      ...
      TXS                     ; correct stack
      BRA   LAB_DATA          ; go perform DATA (find : or [EOL])

; do the return without gosub error

LAB_16F4
      LDX   #$04              ; error code $04 ("RETURN without GOSUB" error)
      BRA   LAB_16F7b         ; branch around next instruction, save one clock cycle
...
LAB_SNBS
      ...
      BRA   LAB_SNBLb         ; branch around next instruction, save one clock cycle
...
LAB_172D
      ...
      BRA   LAB_1725          ; if found go swap search character for alt search character
...
; here to check and ignore the ELSE if present
      ...
      LDA   (Bpntrl)          ; get the next BASIC byte
...
LAB_REM
      JSR   LAB_SNBL          ; scan for next BASIC line
      JMP   LAB_170F          ; go set BASIC execute pointer and return
...
LAB_17B3
      JSR   LAB_IGBY          ; increment and scan memory
      BRA   LAB_1785          ; loop for next character

; perform DEC

LAB_DEC
      LDA   #<LAB_2AFD        ; set -1 pointer low byte
      BRA   LAB_17B5          ; branch around next instruction, saves one clock cycle
...
LAB_17F4
      LDA   des_pl            ; get descriptor pointer low byte
      LDY   des_ph            ; get descriptor pointer high byte
      BRA   LAB_1811          ; clean stack, copy descriptor to variable and return

                              ; make space and copy string
LAB_17FB
      LDA   (des_pl)          ; get string length
...
LAB_IsByte
      ...
      STA   (str_pl)          ; save byte in string (byte IS string!)
LAB_NoSt
      ...
      BRA   LAB_17D5          ; do string LET and return
...
; don't check fit if terminal width byte is zero
      ...
      SBC   (des_pl)          ; subtract string length
      BCS   LAB_185E          ; branch if less than terminal width

      JSR   LAB_CRLF          ; else print CR/LF
LAB_185E
      JSR   LAB_18C6          ; print string from Sutill/Sutilh
      BRA   LAB_182C          ; always go continue processing line
...
LAB_CRLF
      ...
      JSR   LAB_PRNA          ; go print the character
      LDA   #$0A              ; load [LF]
      BRA   LAB_PRNA          ; go print the character and return, branch always
...
LAB_188B
      ...
      BRA   LAB_18BD          ; continue with PRINT processing (branch always)
...
LAB_1898
      ...
      BRA   LAB_18B6          ; always print A spaces (result is never $00)
...
LAB_18E0
      LDA   #$20              ; load " "
      BRA   LAB_PRNA          ; branch around next instruction, saves one clock cycle
...
LAB_19A4
      ...
      BRA   LAB_19B6          ; go check string terminator
...
LAB_19F6
      ...
      BRA   LAB_19DD          ; go find next statement if not "DATA"
...
LAB_NEXT
      ...
      LDY   #$00              ; else clear Y
      BRA   LAB_1A49          ; branch always (no variable to search for)
...
LAB_CTNM
      ...
      BRA   LAB_CKTM          ; branch around SEC
...
LAB_1ACC
      ...
      STZ   comp_f            ; clear compare function flag
LAB_1ADB
      JSR   LAB_GBYT          ; scan memory
LAB_1ADE
      SEC                     ; set carry for subtract
      ...
      JSR   LAB_IGBY          ; increment and scan memory
      BRA   LAB_1ADE          ; go do next character
...
LAB_1B1D
      ...
      BRA   LAB_1B86          ; else pop FAC2 and return, branch always
...
; patched routine - Klaus fixed in 2.22p5 by adding NOP if needed
; this fix corrects the RTS increment by one and handles high byte if needed
...
LAB_1B5B
      ...
      BNE   LAB_1B5B2         ; skip high byte increment if no low byte rollover to $00
...
; LAB_1B66 moved to after JSR LAB_27BA (patch 2.22p5)
...
LAB_SCCA
      CMP   (Bpntrl)          ; check next byte is = A
...
LAB_1BFE
      LDA   #$28              ; load A with "("
      BRA   LAB_SCCA          ; scan for CHR$(A), else do syntax error then warm start
...
TK_EQUAL_PLUS     = TK_EQUAL-TK_PLUS
      ...
      BRA   LAB_1C13          ; do set-up for function then execute (branch always)
...
LAB_1BEE
      ...
      BRA   LAB_SNER          ; else do syntax error
...
LAB_LRMS
      ...
      PLX                     ; get function jump vector low byte
      PLY                     ; get function jump vector high byte
      LDA   des_ph            ; get descriptor pointer high byte
      PHA                     ; push string pointer high byte
      LDA   des_pl            ; get descriptor pointer low byte
      PHA                     ; push string pointer low byte
      PHY                     ; save function jump vector high byte
      PHX                     ; save function jump vector low byte
      JSR   LAB_GTBY          ; get byte parameter
...
LAB_LTHAN
      ...
      TAX                     ; copy result
      BRA   LAB_1CE1          ; go evaluate result
...
LAB_1D77
      ...
      INX                     ; else increment high byte
      BRA   LAB_1D5D          ; loop always (RAM doesn't extend to $FFFF !)
...
LAB_1E85
      ...
      BRA   LAB_1E8A          ; branch around next instruction, saves on clock cycle
...
LAB_1FD0
      LDA   #$00              ; clear high byte
      BRA   LAB_AYFC          ; always save and convert integer AY to FAC1 and return
...
LAB_DEF
      ...
      BRA   LAB_207A          ; put execute pointer and variable pointer into function
                              ; and return
...
LAB_201E
      ...
      PHY                     ; push function pointer high byte
      JSR   LAB_1BFE          ; scan for "(", else do syntax error then warm start
      JSR   LAB_EVEX          ; evaluate expression
      JSR   LAB_1BFB          ; scan for ")", else do syntax error then warm start
      JSR   LAB_CTNM          ; check if source is numeric, else do type mismatch
...
LAB_20F8
      ...
      STZ   des_ph            ; save string descriptor pointer high byte (always $00)
      LDY   #$FF              ; Y = $FF
...
LAB_2137
      ...
      PLA                     ; pull length
      BRA   LAB_2117          ; go try again (loop always, length should never be = $00)
...
LAB_214B
      ...
      STZ   garb_h            ; clear working pointer high byte (flag no strings to move)
                              ; patch 2.22p5
      STZ   garb_l            ; clear working pointer low byte (flag no strings to move)
...
LAB_21CC
      ...
      BRA   LAB_21C4          ; go do next array string (loop always)
...
LAB_224Da                     ; patch 2.22p5 - added extra label
      ...
      LDA   (ssptr_l)         ; get length_1 from descriptor
      CLC                     ; clear carry for add
      ADC   (des_pl)          ; add length_2
...
LAB_LEFT
      ...
      TYA                     ; clear A
      BRA   LAB_2316          ; go do string copy (branch always)
...
LAB_2358
      ...
      BRA   LAB_231E          ; go do string copy (branch always)
...
NoUcase
      ...
      BRA   NoString          ; tidy up and exit, branch always
...
LAB_SUBTRACT
      ...
      STA   FAC_sc            ; save sign compare (FAC1 EOR FAC2)
      LDA   FAC1_e            ; get FAC1 exponent
      BRA   LAB_ADD           ; go add FAC2 to FAC1
...
LAB_2675
      ...
      BRA   LAB_2680b         ; branch around next instruction, saves one clock cycle
LAB_2680
...
LAB_2704
      ...
      BRA   LAB_26F4          ; loop always with no compare
...
LAB_272B
      ...
      BRA   LAB_273C          ; copy temp to FAC1, normalise and return
...
LAB_282E
      BRA   LAB_27D0          ; return A=FF,C=1/-ve A=01,C=0/+ve
...
LAB_2887
      STZ   Dtypef            ; clear data type flag, $FF=string, $00=numeric
      LDX   #$09              ; set index
LAB_288B
      STZ   numexp,X          ; clear byte
      DEX                     ; decrement index
...
; get FAC1 from string .. first character wasn't numeric
      ...
      BRA   LAB_289C          ; branch always (go scan and check for hex/bin)
...
LAB_28C9
      ...
      BRA   LAB_28DD          ; go evaluate exponent
...
LAB_2905
      ...
      JSR   LAB_2912          ; evaluate new ASCII digit
      BRA   LAB_289E          ; go do next character
...
LAB_2934
      ...
      ADC   expcnt            ; * 5
      ASL                     ; * 10
      ADC   (Bpntrl)          ; add character (will be $30 too much!)
      SBC   #'0'-1            ; convert character to binary
LAB_2942
      STA   expcnt            ; save exponent count byte
      BRA   LAB_28C4          ; go get next character
...
LAB_2C74
      ...
      BRA   LAB_2C35          ; go do series evaluation
...
NextB1
      ...
      BRA   GoPr1             ; else go make output string
...
LAB_HEXS
      ...
      ADC   #$06              ; add 6d
      BEQ   GoPr2             ; if zero print whole string
      BRA   GoPr1             ; else go make output string (branch always)
...
LAB_FB95
      STZ   ccnull            ; clear timer because we got a byte
LAB_FB96
      RTS
...
LAB_MAX
      ...
      BRA   LAB_MAX           ; go do next (branch always)
...
LAB_MIN
      ...
      STA   FAC2_1            ; save FAC2 mantissa1
      JSR   LAB_279B          ; copy FAC2 to FAC1
      BRA   LAB_MIN           ; go do next (branch always)
...
;
; The variables used by the C02 Monitor are:
; PTRL/H, plus set the Block count to 1 and clear the OPXMDM flag
;
; LOAD command
...
; and call LAB_1319 to cleanup EhBasic.
...
;
; The Xmodem Save routine in the C02 Monitor expects it's variables
; to be set before making the call via the JMP to V_SAVE.

; The variables used by the C02 Monitor are:
; - Source address (SCRL/H), Length (LENL/H), plus
; - the Block count set to one and the OPXMDM flag cleared.
;
; SAVE command
...
; Basic code. The end is marked by two consecutive $00 bytes.
;
; This routine, starts at (smeml) which points to the next line of
; Basic code, then calculates the address to the next line after that
; and continues till it locates the last line.
;
...
V_EXIT
      JMP   (VEC_EXIT)        ; warm start C02 Monitor
...
;
      .word $FF36             ; byte in from C02BIOS (no waiting)
      .word $FF3C             ; byte out from C02BIOS

      .word $E015             ; Xmodem LOAD for C02 Monitor
      .word $E012             ; Xmodem SAVE for C02 Monitor
      .word $E003             ; Warm start for C02 Monitor

PG4_TABE

; page zero initialisation table $00-$0B inclusive

StrTab
      .byte $4C               ; JMP opcode
      .word LAB_1274          ; warm start vector
      ...
      .byte $50               ; default terminal width byte = 80
...
ERR_OM      .byte "Out of RAM",$00
ERR_US      .byte "Undefined STMT",$00
ERR_BS      .byte "Array bounds",$00
ERR_DD      .byte "Double DIM",$00
