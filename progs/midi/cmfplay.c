...
#ifdef DEBUG_ENABLED
	#define AdPlug_LogWrite(...) printf(__VA_ARGS__)
#else
...
{
	opl2_write(iValue, iRegister);
	this->iCurrentRegs[iRegister] = iValue;
}

void readString(FILE *fd, char *p, uint8_t l){
    fread(p, l, 1, fd);
}

uint16_t readInt(FILE *fd){
    uint16_t v;
    fread(&v, sizeof(uint16_t), 1, fd);
    return v;
}

uint8_t readByte(FILE *fd){
    uint8_t b;
    fread(&b, sizeof(uint8_t), 1, fd);
    return b;
}

void fskip(FILE *fd, uint8_t l){
    while(l-->0)
        readByte(fd);
}
...
void _rewind(int subsong){
  ...
  // Apparently this enables nine-channel mode?
	...
	this->iDelayRemaining = readMIDINumber();
  ...
  // state variables and the instruments are not set correctly!
	...
	memset(this->iNotePlaying, 255, sizeof(this->iNotePlaying));
	memset(this->bNoteFix, false, sizeof(this->bNoteFix));
}
...
bool load(FILE *fd){
	...
	this->cmfHeader.iInstrumentBlockOffset = readInt(fd);
	this->cmfHeader.iMusicOffset = readInt(fd);
	this->cmfHeader.iTicksPerQuarterNote = readInt(fd);
	this->cmfHeader.iTicksPerSecond = readInt(fd);
	this->cmfHeader.iTagOffsetTitle = readInt(fd);
	this->cmfHeader.iTagOffsetComposer = readInt(fd);
	this->cmfHeader.iTagOffsetRemarks = readInt(fd);
	...
	readString(fd, (char *)this->cmfHeader.iChannelsInUse, 16);
	if (iVer == 0x0100) {
		this->cmfHeader.iNumInstruments = readByte(fd);
		this->cmfHeader.iTempo = 0;
	} else { // 0x0101
		this->cmfHeader.iNumInstruments = readInt(fd);
		this->cmfHeader.iTempo = readInt(fd);
	}
    ...
    */
	for (i = 0; i < this->cmfHeader.iNumInstruments; i++) {
		this->pInstruments[i].op[0].iCharMult = readByte(fd);
		this->pInstruments[i].op[1].iCharMult = readByte(fd);
		this->pInstruments[i].op[0].iScalingOutput = readByte(fd);
		this->pInstruments[i].op[1].iScalingOutput = readByte(fd);
		this->pInstruments[i].op[0].iAttackDecay = readByte(fd);
		this->pInstruments[i].op[1].iAttackDecay = readByte(fd);
		this->pInstruments[i].op[0].iSustainRelease = readByte(fd);
		this->pInstruments[i].op[1].iSustainRelease = readByte(fd);
		this->pInstruments[i].op[0].iWaveSel = readByte(fd);
		this->pInstruments[i].op[1].iWaveSel = readByte(fd);
		this->pInstruments[i].iConnection = readByte(fd);
		fskip(fd, 5);  // skip over the padding bytes
	}

	// Set the rest of the instruments to the CMF defaults
	for (i = this->cmfHeader.iNumInstruments; i < 128; i++) {
...
void cmfNoteOff(uint8_t iChannel, uint8_t iNote, uint8_t iVelocity)
{
	...
	if ((iChannel > 10) && (this->bPercussive)) {
		...
		writeOPL(BASE_RHYTHM, this->iCurrentRegs[BASE_RHYTHM] & ~(1 << (15 - iChannel)));
		...
		writeOPL(BASE_KEYON_FREQ + iOPLChannel, this->iCurrentRegs[BASE_KEYON_FREQ + iOPLChannel] & ~OPLBIT_KEYON);
	}
	return;
}
...
void cmfNoteUpdate(uint8_t iChannel)
{
	...
	if ((iChannel > 10) && (this->bPercussive)) {
		uint8_t iPercChannel = getPercChannel(iChannel);
		getFreq(iChannel, this->chOPL[iPercChannel].iMIDINote, &iBlock, &iOPLFNum);

		// Update note frequency
		writeOPL(BASE_FNUM_L + iPercChannel, iOPLFNum & 0xFF);
		writeOPL(BASE_KEYON_FREQ + iPercChannel, (iBlock << 2) | ((iOPLFNum >> 8) & 0x03));

	} else { // Non rhythm-mode or a normal instrument channel

		// Figure out which OPL channels should be updated
		iNumChannels = this->bPercussive ? 6 : 9;
		for (i = 0; i < iNumChannels; i++) {
			...
			if (this->chOPL[i].iMIDIChannel == iChannel && this->chOPL[i].iNoteStart > 0) {
				...
				writeOPL(BASE_FNUM_L + i, iOPLFNum & 0xFF);
				writeOPL(BASE_KEYON_FREQ + i, OPLBIT_KEYON | (iBlock << 2) | ((iOPLFNum & 0x300) >> 8));
			}
		}
	}
	return;
...
void writeInstrumentSettings(uint8_t iChannel, uint8_t iOperatorSource, uint8_t iOperatorDest, uint8_t iInstrument)
{
	...
	writeOPL(BASE_CHAR_MULT + iOPLOffset, this->pInstruments[iInstrument].op[iOperatorSource].iCharMult);
	writeOPL(BASE_SCAL_LEVL + iOPLOffset, this->pInstruments[iInstrument].op[iOperatorSource].iScalingOutput);
	writeOPL(BASE_ATCK_DCAY + iOPLOffset, this->pInstruments[iInstrument].op[iOperatorSource].iAttackDecay);
	writeOPL(BASE_SUST_RLSE + iOPLOffset, this->pInstruments[iInstrument].op[iOperatorSource].iSustainRelease);
	writeOPL(BASE_WAVE      + iOPLOffset, this->pInstruments[iInstrument].op[iOperatorSource].iWaveSel);

	// TODO: Check to see whether we should only be loading this for one or both operators
	writeOPL(BASE_FEED_CONN + iChannel, this->pInstruments[iInstrument].iConnection);
	return;
}
...
void MIDIcontroller(uint8_t iChannel, uint8_t iController, uint8_t iValue)
{
	switch (iController) {
		case 0x63:
			...
			//   3 == AM+VIB on
			if (iValue) {
				writeOPL(BASE_RHYTHM, (this->iCurrentRegs[BASE_RHYTHM] & ~0xC0) | (iValue << 6)); // switch AM+VIB extension on
			} else {
				writeOPL(BASE_RHYTHM, this->iCurrentRegs[BASE_RHYTHM] & ~0xC0); // switch AM+VIB extension off
			}
		...
		case 0x67:
			...
			if (this->bPercussive) {
				writeOPL(BASE_RHYTHM, this->iCurrentRegs[BASE_RHYTHM] | 0x20); // switch rhythm-mode on
			} else {
				writeOPL(BASE_RHYTHM, this->iCurrentRegs[BASE_RHYTHM] & ~0x20); // switch rhythm-mode off
			}
...
void cmfNoteOn(uint8_t iChannel, uint8_t iNote, uint8_t iVelocity)
{
	...
	if ((iChannel > 10) && (this->bPercussive)) {
		...
		//if (this->chOPL[iPercChannel].iMIDIPatch != this->chMIDI[iChannel].iPatch) {
			MIDIchangeInstrument(iPercChannel, iChannel, this->chMIDI[iChannel].iPatch);
		//}
		...
		*/
		...
		iLevel = 0x25 ;//TODO FIXME - sqrt(iVelocity * 16/*6*/);//(127 - iVelocity) * 0x20 / 127;
		if (iVelocity > 0x7b) iLevel = 0; // full volume
		if (iLevel < 0) iLevel = 0;
		if (iLevel > 0x3F) iLevel = 0x3F;
		//if (iVelocity < 0x40) iLevel = 0x10;

		iOPLOffset = BASE_SCAL_LEVL + OPLOFFSET(iPercChannel);
		...
		writeOPL(iOPLOffset, (this->iCurrentRegs[iOPLOffset] & ~0x3F) | iLevel);//(iVelocity * 0x3F / 127));
		//}
		// Bass drum (ch11) uses both operators
		//if (iChannel == 11) writeOPL(iOPLOffset + 3, (this->iCurrentRegs[iOPLOffset + 3] & ~0x3F) | iLevel);
...
/*		#ifdef USE_VELOCITY  // Official CMF player seems to ignore velocity levels
			...
			//writeOPL(iOPLOffset, (this->iCurrentRegs[iOPLOffset] & ~0x3F) | (0x3F - (iVelocity >> 1)));//(iVelocity * 0x3F / 127));
			writeOPL(iOPLOffset, (this->iCurrentRegs[iOPLOffset] & ~0x3F) | iLevel);//(iVelocity * 0x3F / 127));
		#endif*/
		...
		if (this->iCurrentRegs[BASE_RHYTHM] & iBit) writeOPL(BASE_RHYTHM, this->iCurrentRegs[BASE_RHYTHM] & ~iBit);

		// I wonder whether we need to delay or anything here?

		// Turn the note on
		//if (iChannel == 15) {
		writeOPL(BASE_RHYTHM, this->iCurrentRegs[BASE_RHYTHM] | iBit);
		...
		if (iOPLChannel == -1) {
			...
			iOPLChannel = 0;
			iEarliest = this->chOPL[0].iNoteStart;
			for (i = 1; i < iNumChannels; i++) {
		...
		if (this->chOPL[iOPLChannel].iMIDIPatch != this->chMIDI[iChannel].iPatch) {
			MIDIchangeInstrument(iOPLChannel, iChannel, this->chMIDI[iChannel].iPatch);
		}
		...
		this->chOPL[iOPLChannel].iMIDINote = iNote;
			...
			uint16_t iLevel = 0x2F - (iVelocity * 0x2F / 127); // 0x2F should be 0x3F but it's too quiet then
			writeOPL(iOPLOffset, (this->iCurrentRegs[iOPLOffset] & ~0x3F) | iLevel);
		#endif

		// Set the frequency and play the note
		writeOPL(BASE_FNUM_L + iOPLChannel, iOPLFNum & 0xFF);
		writeOPL(BASE_KEYON_FREQ + iOPLChannel, OPLBIT_KEYON | (iBlock << 2) | ((iOPLFNum & 0x300) >> 8));
	}
	return;
}
...
bool update()
{
	...
	while (!this->iDelayRemaining) {
		...
		iChannel = iCommand & 0x0F;
		switch (iCommand & 0xF0) {
			case 0x80: { // Note off (two data bytes)
				if (this->iPlayPointer < this->iSongLen - 1) break;
				iNote = this->data[this->iPlayPointer++];
				iVelocity = this->data[this->iPlayPointer++]; // release velocity
			...
			case 0xA0: { // Polyphonic key pressure (two data bytes)
				if (this->iPlayPointer > this->iSongLen - 2) break;
				iNote = this->data[this->iPlayPointer++];
				iPressure = this->data[this->iPlayPointer++];
			...
			case 0xB0: { // Controller (two data bytes)
				if (this->iPlayPointer > this->iSongLen - 2) break;
				iController = this->data[this->iPlayPointer++];
				iValue = this->data[this->iPlayPointer++];
			...
			case 0xC0: { // Instrument change (one data byte)
				...
				iNewInstrument = this->data[this->iPlayPointer++];
			...
			case 0xE0: { // Pitch bend (two data bytes)
				if (this->iPlayPointer > this->iSongLen - 2) break;
				iLSB = this->data[this->iPlayPointer++];
				iMSB = this->data[this->iPlayPointer++];
				iValue16 = (iMSB << 7) | iLSB;
				// 8192 is middle/off, 0 is -2 semitones, 16384 is +2 semitones
				this->chMIDI[iChannel].iPitchbend = iValue16;
				cmfNoteUpdate(iChannel);
//				AdPlug_LogWrite("CMF: Channel %d pitchbent to %d (%+.2f)\n", iChannel + 1, iValue16, (float)(iValue16 - 8192) / 8192);
				break;
			}
			case 0xF0: // System message (arbitrary data bytes)
				switch (iCommand) {
					...
					case 0xFF: { // System reset, used as meta-events in a MIDI file
						if (this->iPlayPointer >= this->iSongLen) break;
						iEvent = this->data[this->iPlayPointer++];
						switch (iEvent) {
...
int main(int argc, char* argv[]){
    ...
    if(load(fd)){
        fclose(fd);
        fprintf(stderr, "not a CMF file '%s'\n", argv[1]);
        return EXIT_FAILURE;
    }    
